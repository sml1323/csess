...
## 잠긴 결정 15 — 검색 매치 지점에 앵커되는 프리뷰 (2026-07-03, spec-kit)

본문 검색으로 세션은 찾지만 "매치가 대화 어디에 있는지"가 안 보이던 문제. 선택하면 프리뷰가 항상
맨 위였고 Ctrl+D/U 로 수동으로 훑어야 했다. → 검색어가 있으면 프리뷰를 **첫 매치 지점으로 앵커**(위로
2줄 맥락)하고 매치 단어를 **강조**, `alt-n/alt-p` 로 매치 순회.

- **스크롤 = 논리 라인 인덱스(1:1).** 프리뷰 `Paragraph` 는 `.wrap()` 없음 → 매치 라인 인덱스를 그대로
  `preview_scroll` 로. 앵커는 리셋 지점(`recompute`/`move_sel`)에만 끼워, Ctrl+D/U 는 안 거치므로
  **수동 스크롤이 우선**(별도 플래그 불필요).
- **매치는 검색 body 가 아니라 렌더된 라인 텍스트에서 찾는다.** 검색 코퍼스(제목+text 블록)와 프리뷰
  (접힘/생략/상한)가 달라, 안 보이는 매치(슬래시 래퍼·상한 밖)는 자연히 **top 폴백**.
- **하이라이트는 (path,query) 캐시.** base 렌더는 path 캐시 유지, 하이라이트만 별도 캐시 → 스크롤 중
  query 불변이라 재계산 0(D14 스크롤 idle 계승). 대소문자 무시 매치를 원본 바이트 오프셋으로 되매핑해
  분할(ß/İ 등 길이 변화에도 char 경계 안전).

전체 스펙·설계·태스크: [`specs/001-search-hit-preview/`](../specs/001-search-hit-preview/) (spec/plan/research/data-model/contracts/tasks).
